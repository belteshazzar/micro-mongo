...
function isIn(val, values3) {
  for (var i = 0; i < values3.length; i++) {
    if (valuesEqual$1(values3[i], val)) return true;
  }
  return false;
}
...
class Cursor {
  constructor(collection, query5, projection, matches2, storage, indexes, planQuery, SortedCursor2) {
...
class SortedCursor {
  constructor(collection, query5, cursor, sort3) {
    this.collection = collection;
    this.query = query5;
    this.sortSpec = sort3;
    this.pos = 0;
    ...
    const sortKeys = Object.keys(sort3);
    this.items.sort(function(a, b) {
      for (let i = 0; i < sortKeys.length; i++) {
        if (a[sortKeys[i]] == void 0 && b[sortKeys[i]] != void 0) return -1 * sort3[sortKeys[i]];
        if (a[sortKeys[i]] != void 0 && b[sortKeys[i]] == void 0) return 1 * sort3[sortKeys[i]];
        if (a[sortKeys[i]] < b[sortKeys[i]]) return -1 * sort3[sortKeys[i]];
        if (a[sortKeys[i]] > b[sortKeys[i]]) return 1 * sort3[sortKeys[i]];
      }
      return 0;
    });
  ...
  limit(max3) {
    this.items = this.items.slice(0, max3);
    return this;
  }
  map(fn) {
    const results = [];
...
function requireEarcut() {
  ...
  function earcut2(data, holeIndices, dim) {
    ...
    var minX2, minY2, maxX2, maxY2, x, y, invSize;
    ...
    earcutLinked2(outerNode, triangles, dim, minX2, minY2, invSize);
    return triangles;
  }
  function linkedList2(data, start2, end2, dim, clockwise) {
    var i, last;
    if (clockwise === signedArea5(data, start2, end2, dim) > 0) {
  ...
  function earcutLinked2(ear, triangles, dim, minX2, minY2, invSize, pass) {
    if (!ear) return;
    if (!pass && invSize) indexCurve2(ear, minX2, minY2, invSize);
    var stop = ear, prev, next3;
    while (ear.prev !== ear.next) {
      prev = ear.prev;
      next3 = ear.next;
      if (invSize ? isEarHashed2(ear, minX2, minY2, invSize) : isEar2(ear)) {
      ...
      if (ear === stop) {
        if (!pass) {
          earcutLinked2(filterPoints2(ear), triangles, dim, minX2, minY2, invSize, 1);
        } else if (pass === 1) {
          ear = cureLocalIntersections2(ear, triangles, dim);
          earcutLinked2(ear, triangles, dim, minX2, minY2, invSize, 2);
        } else if (pass === 2) {
          splitEarcut2(ear, triangles, dim, minX2, minY2, invSize);
        }
        break;
      }
    }
  ...
  function isEar2(ear) {
    ...
    if (area5(a, b, c) >= 0) return false;
    var p = ear.next.next;
    while (p !== ear.prev) {
      if (pointInTriangle2(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) && area5(p.prev, p, p.next) >= 0) return false;
      p = p.next;
    }
    return true;
  }
  function isEarHashed2(ear, minX2, minY2, invSize) {
    var a = ear.prev, b = ear, c = ear.next;
    if (area5(a, b, c) >= 0) return false;
    var minTX = a.x < b.x ? a.x < c.x ? a.x : c.x : b.x < c.x ? b.x : c.x, minTY = a.y < b.y ? a.y < c.y ? a.y : c.y : b.y < c.y ? b.y : c.y, maxTX = a.x > b.x ? a.x > c.x ? a.x : c.x : b.x > c.x ? b.x : c.x, maxTY = a.y > b.y ? a.y > c.y ? a.y : c.y : b.y > c.y ? b.y : c.y;
    var minZ = zOrder2(minTX, minTY, minX2, minY2, invSize), maxZ = zOrder2(maxTX, maxTY, minX2, minY2, invSize);
    var p = ear.prevZ, n = ear.nextZ;
    while (p && p.z >= minZ && n && n.z <= maxZ) {
      if (p !== ear.prev && p !== ear.next && pointInTriangle2(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) && area5(p.prev, p, p.next) >= 0) return false;
      p = p.prevZ;
      if (n !== ear.prev && n !== ear.next && pointInTriangle2(a.x, a.y, b.x, b.y, c.x, c.y, n.x, n.y) && area5(n.prev, n, n.next) >= 0) return false;
      n = n.nextZ;
    }
    while (p && p.z >= minZ) {
      if (p !== ear.prev && p !== ear.next && pointInTriangle2(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) && area5(p.prev, p, p.next) >= 0) return false;
  ...
  function cureLocalIntersections2(start2, triangles, dim) {
    ...
    do {
      ...
      if (!equals10(a, b) && intersects7(a, p, p.next, b) && locallyInside2(a, b) && locallyInside2(b, a)) {
  ...
  function splitEarcut2(start2, triangles, dim, minX2, minY2, invSize) {
    var a = start2;
    do {
      var b = a.next.next;
      while (b !== a.prev) {
        if (a.i !== b.i && isValidDiagonal2(a, b)) {
          ...
          earcutLinked2(a, triangles, dim, minX2, minY2, invSize);
          earcutLinked2(c, triangles, dim, minX2, minY2, invSize);
          return;
        }
        b = b.next;
      }
  ...
  function indexCurve2(start2, minX2, minY2, invSize) {
    var p = start2;
    do {
      if (p.z === null) p.z = zOrder2(p.x, p.y, minX2, minY2, invSize);
      p.prevZ = p.prev;
      p.nextZ = p.next;
      p = p.next;
    } while (p !== start2);
  ...
  function zOrder2(x, y, minX2, minY2, invSize) {
    x = 32767 * (x - minX2) * invSize;
    y = 32767 * (y - minY2) * invSize;
    x = (x | x << 8) & 16711935;
    x = (x | x << 4) & 252645135;
    x = (x | x << 2) & 858993459;
    x = (x | x << 1) & 1431655765;
  ...
  function intersects7(p1, q1, p2, q2) {
    if (equals10(p1, q1) && equals10(p2, q2) || equals10(p1, q2) && equals10(p2, q1)) return true;
    return area5(p1, q1, p2) > 0 !== area5(p1, q1, q2) > 0 && area5(p2, q2, p1) > 0 !== area5(p2, q2, q1) > 0;
  }
  ...
  function middleInside2(a, b) {
    var p = a, inside3 = false, px = (a.x + b.x) / 2, py = (a.y + b.y) / 2;
    do {
  ...
  earcut2.deviation = function(data, holeIndices, dim, triangles) {
    ...
    var polygonArea2 = Math.abs(signedArea5(data, 0, outerLen, dim));
    if (hasHoles) {
      for (var i = 0, len = holeIndices.length; i < len; i++) {
        var start2 = holeIndices[i] * dim;
        var end2 = i < len - 1 ? holeIndices[i + 1] * dim : data.length;
        polygonArea2 -= Math.abs(signedArea5(data, start2, end2, dim));
      }
    }
    var trianglesArea = 0;
  ...
  function signedArea5(data, start2, end2, dim) {
    var sum = 0;
...
function lengthToRadians(distance11, units) {
  if (distance11 === void 0 || distance11 === null) throw new Error("distance is required");
  if (units && typeof units !== "string") throw new Error("units must be a string");
  var factor = factors[units || "kilometers"];
  if (!factor) throw new Error(units + " units is invalid");
...
function coordEach$1(geojson, callback, excludeWrapCoord) {
  ...
  var j, k, l, geometry$$1, stopG, coords, geometryMaybeCollection, wrapShrink = 0, coordIndex = 0, isGeometryCollection2, type2 = geojson.type, isFeatureCollection = type2 === "FeatureCollection", isFeature = type2 === "Feature", stop = isFeatureCollection ? geojson.features.length : 1;
  for (var featureIndex = 0; featureIndex < stop; featureIndex++) {
    ...
    for (var geomIndex = 0; geomIndex < stopG; geomIndex++) {
      ...
      var geometryIndex = 0;
      geometry$$1 = isGeometryCollection2 ? geometryMaybeCollection.geometries[geomIndex] : geometryMaybeCollection;
...
function geomEach(geojson, callback) {
  var i, j, g, geometry$$1, stopG, geometryMaybeCollection, isGeometryCollection2, featureProperties, featureBBox, featureId, featureIndex = 0, isFeatureCollection = geojson.type === "FeatureCollection", isFeature = geojson.type === "Feature", stop = isFeatureCollection ? geojson.features.length : 1;
  for (i = 0; i < stop; i++) {
    ...
    stopG = isGeometryCollection2 ? geometryMaybeCollection.geometries.length : 1;
    for (g = 0; g < stopG; g++) {
      geometry$$1 = isGeometryCollection2 ? geometryMaybeCollection.geometries[g] : geometryMaybeCollection;
      if (geometry$$1 === null) {
...
function flattenEach(geojson, callback) {
  geomEach(geojson, function(geometry$$1, featureIndex, properties, bbox2, id) {
    ...
    geometry$$1.coordinates.forEach(function(coordinate2, multiFeatureIndex) {
      var geom = {
        type: geomType,
...
function partialSort(arr, k, left, right, compare10) {
  left = left || 0;
  right = right || arr.length - 1;
  compare10 = compare10 || defaultCompare$1;
  while (right > left) {
    if (right - left > 600) {
      var n = right - left + 1;
      var m = k - left + 1;
      ...
      partialSort(arr, k, newLeft, newRight, compare10);
    }
    var t = arr[k];
    var i = left;
    var j = right;
    swap$1(arr, left, k);
    if (compare10(arr[right], t) > 0) swap$1(arr, left, right);
    while (i < j) {
      swap$1(arr, i, j);
      i++;
      j--;
    ...
    if (compare10(arr[left], t) === 0) swap$1(arr, left, j);
    else {
      j++;
      swap$1(arr, j, right);
    }
...
rbush$1.prototype = {
  ...
  _chooseSubtree: function(bbox2, node, level, path) {
    ...
    while (true) {
      ...
      for (i = 0, len = node.children.length; i < len; i++) {
        child = node.children[i];
        area5 = bboxArea$1(child);
        enlargement = enlargedArea$1(bbox2, child) - area5;
  ...
  _chooseSplitIndex: function(node, m, M) {
    ...
    for (i = m; i <= M - m; i++) {
      ...
      overlap2 = intersectionArea$1(bbox1, bbox2);
      area5 = bboxArea$1(bbox1) + bboxArea$1(bbox2);
...
function multiSelect$1(arr, left, right, n, compare10) {
  ...
  while (stack.length) {
    ...
    quickselect$1(arr, mid, left, right, compare10);
...
function scaleLinearExpansion(e, scale3) {
  var n = e.length;
  if (n === 1) {
    var ts = twoProduct_1(e[0], scale3);
    if (ts[0]) {
      return ts;
    }
    return [ts[1]];
  ...
  var t = [0.1, 0.1];
  var count2 = 0;
  twoProduct_1(e[0], scale3, q);
  if (q[0]) {
    g[count2++] = q[0];
  }
  for (var i = 1; i < n; ++i) {
    twoProduct_1(e[i], scale3, t);
    var pq = q[1];
    twoSum(pq, t[0], q);
    if (q[0]) {
      g[count2++] = q[0];
...
var orientation_1 = createCommonjsModule(function(module) {
  ...
  var CACHED = [
    ...
    function orientation4(a, b, c, d) {
      ...
      var adxbdy = adx * bdy;
      var bdxady = bdx * ady;
      var det2 = adz * (bdxcdy - cdxbdy) + bdz * (cdxady - adxcdy) + cdz * (adxbdy - bdxady);
  ...
  function slowOrient(args) {
    ...
    if (!proc) {
      proc = CACHED[args.length] = orientation2(args.length);
    }
...
function TinyQueue(data, compare10) {
  if (!(this instanceof TinyQueue)) return new TinyQueue(data, compare10);
  this.data = data || [];
  this.length = this.data.length;
  this.compare = compare10 || defaultCompare$1$1;
  if (this.length > 0) {
...
TinyQueue.prototype = {
  ...
  _down: function(pos) {
    ...
    while (pos < halfLength) {
      ...
      if (compare10(best, item) >= 0) break;
      data[pos] = best;
      pos = left;
    }
    data[pos] = item;
...
var pointInPolygon = function(point4, vs) {
...
function booleanPointInPolygon(point4, polygon4, options) {
  ...
  if (!point4) throw new Error("point is required");
  if (!polygon4) throw new Error("polygon is required");
  var pt = getCoord(point4);
  var polys = getCoords(polygon4);
  var type2 = polygon4.geometry ? polygon4.geometry.type : polygon4.type;
...
function destination(origin, distance11, bearing2, options) {
  ...
  var radians = lengthToRadians(distance11, units);
...
function earcut(data, holeIndices, dim) {
  ...
  earcutLinked(outerNode, triangles, dim, minX2, minY2, invSize);
  return triangles;
}
...
function isEarHashed(ear, minX2, minY2, invSize) {
  ...
  if (area(a, b, c) >= 0) return false;
  var minTX = a.x < b.x ? a.x < c.x ? a.x : c.x : b.x < c.x ? b.x : c.x, minTY = a.y < b.y ? a.y < c.y ? a.y : c.y : b.y < c.y ? b.y : c.y, maxTX = a.x > b.x ? a.x > c.x ? a.x : c.x : b.x > c.x ? b.x : c.x, maxTY = a.y > b.y ? a.y > c.y ? a.y : c.y : b.y > c.y ? b.y : c.y;
  var minZ = zOrder(minTX, minTY, minX2, minY2, invSize), maxZ = zOrder(maxTX, maxTY, minX2, minY2, invSize);
  var p = ear.nextZ;
  while (p && p.z <= maxZ) {
  ...
  do {
    ...
    while (b !== a.prev) {
      if (a.i !== b.i && isValidDiagonal(a, b)) {
        ...
        earcutLinked(a, triangles, dim, minX2, minY2, invSize);
        earcutLinked(c, triangles, dim, minX2, minY2, invSize);
        return;
      }
      b = b.next;
    }
...
function indexCurve(start2, minX2, minY2, invSize) {
  var p = start2;
  do {
    if (p.z === null) p.z = zOrder(p.x, p.y, minX2, minY2, invSize);
    p.prevZ = p.prev;
    p.nextZ = p.next;
    p = p.next;
  } while (p !== start2);
...
function zOrder(x, y, minX2, minY2, invSize) {
  x = 32767 * (x - minX2) * invSize;
  y = 32767 * (y - minY2) * invSize;
  x = (x | x << 8) & 16711935;
  x = (x | x << 4) & 252645135;
...
function quickselect$3(arr, k, left, right, compare10) {
  quickselectStep$1(arr, k, left || 0, right || arr.length - 1, compare10 || defaultCompare$2);
}
function quickselectStep$1(arr, k, left, right, compare10) {
  while (right > left) {
    if (right - left > 600) {
      var n = right - left + 1;
      var m = k - left + 1;
      ...
      var newLeft = Math.max(left, Math.floor(k - m * s / n + sd));
      var newRight = Math.min(right, Math.floor(k + (n - m) * s / n + sd));
      quickselectStep$1(arr, k, newLeft, newRight, compare10);
    }
    var t = arr[k];
    var i = left;
    var j = right;
    swap$1$1(arr, left, k);
    if (compare10(arr[right], t) > 0) swap$1$1(arr, left, right);
    while (i < j) {
      swap$1$1(arr, i, j);
      i++;
      j--;
    ...
    if (compare10(arr[left], t) === 0) swap$1$1(arr, left, j);
    else {
      j++;
...
rbush$4.prototype = {
  ...
  _chooseSubtree: function(bbox2, node, level, path) {
    ...
    while (true) {
      ...
      for (i = 0, len = node.children.length; i < len; i++) {
        child = node.children[i];
        area5 = bboxArea$1$1(child);
        enlargement = enlargedArea$1$1(bbox2, child) - area5;
        if (enlargement < minEnlargement) {
          minEnlargement = enlargement;
          minArea = area5 < minArea ? area5 : minArea;
          targetNode = child;
  ...
  _chooseSplitIndex: function(node, m, M) {
    ...
    for (i = m; i <= M - m; i++) {
      ...
      overlap2 = intersectionArea$1$1(bbox1, bbox2);
      area5 = bboxArea$1$1(bbox1) + bboxArea$1$1(bbox2);
      if (overlap2 < minOverlap) {
        minOverlap = overlap2;
        index2 = i;
        minArea = area5 < minArea ? area5 : minArea;
...
function multiSelect$1$1(arr, left, right, n, compare10) {
  var stack = [left, right], mid;
  while (stack.length) {
    right = stack.pop();
    left = stack.pop();
    if (right - left <= n) continue;
    mid = left + Math.ceil((right - left) / n / 2) * n;
    quickselect$3(arr, mid, left, right, compare10);
    stack.push(left, mid, mid, right);
  }
}
  ...
  switch (geojson.type) {
    ...
    case "MultiPolygon":
      for (i = 0; i < geojson.coordinates.length; i++) {
        area5 += polygonArea(geojson.coordinates[i]);
      }
      return area5;
    case "Point":
    case "MultiPoint":
    ...
    case "GeometryCollection":
      for (i = 0; i < geojson.geometries.length; i++) {
        area5 += calculateArea(geojson.geometries[i]);
      }
      return area5;
  }
}
function polygonArea(coords) {
  ...
  if (coords && coords.length > 0) {
    area5 += Math.abs(ringArea(coords[0]));
    for (var i = 1; i < coords.length; i++) {
      area5 -= Math.abs(ringArea(coords[i]));
    }
  }
  return area5;
}
function ringArea(coords) {
  ...
  if (coordsLength > 2) {
    for (i = 0; i < coordsLength; i++) {
      ...
      area5 += (rad(p3[0]) - rad(p1[0])) * Math.sin(rad(p2[1]));
    }
    area5 = area5 * RADIUS * RADIUS / 2;
  }
  return area5;
}
function rad(_) {
...
function polygonclip(points, bbox2) {
  var result, edge, prev, prevInside, i, p, inside3;
  for (edge = 1; edge <= 8; edge *= 2) {
    ...
    for (i = 0; i < points.length; i++) {
      p = points[i];
      inside3 = !(bitCode(p, bbox2) & edge);
      if (inside3 !== prevInside) result.push(intersect$1(prev, p, edge, bbox2));
      if (inside3) result.push(p);
      prev = p;
      prevInside = inside3;
    }
    points = result;
...
function envelopeContains(self, env) {
  return env.geometry.coordinates[0].every(function(c) {
    return booleanPointInPolygon(point$2(c), self);
  });
}
...
var keys = createCommonjsModule(function(module, exports$1) {
  exports$1 = module.exports = typeof Object.keys === "function" ? Object.keys : shim;
  exports$1.shim = shim;
  function shim(obj) {
    var keys2 = [];
...
keys.shim;
var is_arguments = createCommonjsModule(function(module, exports$1) {
...
var DBSCAN_1 = createCommonjsModule(function(module) {
  ...
  DBSCAN.prototype._init = function(dataset, epsilon2, minPts, distance11) {
    if (dataset) {
...
var OPTICS_1 = createCommonjsModule(function(module) {
  if (module.exports) {
    var PriorityQueue3 = PriorityQueue_1;
  }
  ...
  OPTICS.prototype._init = function(dataset, epsilon2, minPts, distance11) {
    if (dataset) {
      if (!(dataset instanceof Array)) {
        throw Error("Dataset must be of type array, " + typeof dataset + " given");
      }
...
function removeBeach(beach) {
  var circle = beach.circle, x = circle.x, y = circle.cy, vertex = [x, y], previous = beach.P, next3 = beach.N, disappearing = [beach];
  detachBeach(beach);
  var lArc = previous;
...
CoordinateSequence.prototype.copy = function copy4() {
};
...
var extract$1 = {
   ...
   */
  polygon: function polygon(polygon$12) {
    var this$1$1 = this;
    var array = [];
...
var parse$1 = {
  ...
  multipolygon: function multipolygon2(str) {
    ...
    for (var i = 0, len = polygons.length; i < len; ++i) {
      polygon4 = polygons[i].replace(regExes.trimParens, "$1");
...
Geometry.prototype.getEnvelope = function getEnvelope2() {
...
Collection$1.prototype.add = function add2() {
};
...
Collection$1.prototype.size = function size2() {
};
...
var ArrayList = (function(List$$1) {
  ...
  ArrayList2.prototype.add = function add15(e) {
    if (arguments.length === 1) {
      this.array_.push(e);
    } else {
  ...
  ArrayList2.prototype.toArray = function toArray3() {
    var this$1$1 = this;
    var array = [];
...
var CoordinateList = (function(ArrayList$$1) {
  ...
  CoordinateList2.prototype.clone = function clone5() {
    ...
    for (var i = 0; i < this.size(); i++) {
      clone6.add(i, this$1$1.get(i).copy());
    }
    return clone6;
  };
  ...
  CoordinateList2.prototype.add = function add15() {
    var this$1$1 = this;
    if (arguments.length === 1) {
      var coord = arguments[0];
...
var HashSet = (function(Set$$1) {
  ...
  HashSet2.prototype.add = function add15(o) {
    if (this.contains(o)) {
      return false;
    }
    this.array_.push(o);
  ...
  HashSet2.prototype.toArray = function toArray3() {
    var this$1$1 = this;
    var array = [];
    for (var i = 0, len = this.array_.length; i < len; i++) {
      array.push(this$1$1.array_[i]);
...
var GeometryCollection = (function(Geometry$$1) {
  ...
  GeometryCollection2.prototype.apply = function apply() {
    ...
    if (hasInterface(arguments[0], CoordinateFilter)) {
      ...
      for (var i = 0; i < this._geometries.length; i++) {
        this$1$1._geometries[i].apply(filter17);
      }
...
CoordinateSequences.copy = function copy5(src, srcPos, dest, destPos, length) {
...
var LineString$1 = (function(Geometry$$1) {
  ...
  LineString.prototype.isRing = function isRing3() {
    return this.isClosed() && this.isSimple();
  };
  LineString.prototype.getSortIndex = function getSortIndex() {
    return Geometry$$1.SORTINDEX_LINESTRING;
  };
  LineString.prototype.getCoordinates = function getCoordinates11() {
    return this._points.toCoordinateArray();
  };
  LineString.prototype.equalsExact = function equalsExact2() {
    var this$1$1 = this;
    if (arguments.length === 2) {
      var other = arguments[0];
      var tolerance = arguments[1];
  ...
  LineString.prototype.normalize = function normalize4() {
    var this$1$1 = this;
  ...
  LineString.prototype.getCoordinate = function getCoordinate18() {
    if (this.isEmpty()) {
      return null;
    }
  ...
  LineString.prototype.isClosed = function isClosed5() {
    if (this.isEmpty()) {
      return false;
    }
  ...
  LineString.prototype.getDimension = function getDimension3() {
    return 1;
  };
  LineString.prototype.getLength = function getLength3() {
    return CGAlgorithms.computeLength(this._points);
  };
  LineString.prototype.getNumPoints = function getNumPoints() {
    return this._points.size();
  };
  LineString.prototype.reverse = function reverse4() {
  ...
  LineString.prototype.apply = function apply() {
    ...
    if (hasInterface(arguments[0], CoordinateFilter)) {
      var filter17 = arguments[0];
      for (var i = 0; i < this._points.size(); i++) {
        filter17.filter(this$1$1._points.getCoordinate(i));
      }
    } else if (hasInterface(arguments[0], CoordinateSequenceFilter)) {
      var filter$1 = arguments[0];
  ...
  LineString.prototype.getBoundary = function getBoundary3() {
    return new BoundaryOp(this).getBoundary();
  };
  LineString.prototype.isEquivalentClass = function isEquivalentClass(other) {
    return other instanceof LineString;
  };
  LineString.prototype.clone = function clone5() {
  ...
  LineString.prototype.copy = function copy8() {
    return new LineString(this._points.copy(), this._factory);
  };
  LineString.prototype.getCoordinateSequence = function getCoordinateSequence() {
    return this._points;
  };
  LineString.prototype.isEmpty = function isEmpty6() {
    return this._points.size() === 0;
  };
  LineString.prototype.init = function init8(points) {
    if (points === null) {
  ...
  LineString.prototype.interfaces_ = function interfaces_170() {
    return [Lineal];
  };
  LineString.prototype.getClass = function getClass169() {
    return LineString;
  };
  staticAccessors2.serialVersionUID.get = function() {
...
var Polygon = (function(Geometry$$1) {
  ...
  Polygon2.prototype.apply = function apply(filter17) {
    ...
    if (hasInterface(filter17, CoordinateFilter)) {
      ...
      this._shell.apply(filter17);
      for (var i = 0; i < this._holes.length; i++) {
        this$1$1._holes[i].apply(filter17);
      }
    }
  };
...
var MultiPoint = (function(GeometryCollection$$1) {
  ...
  MultiPoint2.prototype.isValid = function isValid8() {
    return true;
  };
...
GeometryEditor.prototype.editPolygon = function editPolygon(polygon4, operation) {
  var this$1$1 = this;
  var newPolygon = operation.edit(polygon4, this._factory);
  if (newPolygon === null) {
...
CoordinateArraySequence.prototype.copy = function copy6() {
  var this$1$1 = this;
  var cloneCoordinates = new Array(this.size()).fill(null);
  for (var i = 0; i < this._coordinates.length; i++) {
...
GeoJSONParser.prototype.read = function read2(json) {
  ...
  if (geometryTypes.indexOf(type2) !== -1) {
    return parse2[type2].apply(this, [obj.coordinates]);
  } else if (type2 === "GeometryCollection") {
    return parse2[type2].apply(this, [obj.geometries]);
  }
  return parse2[type2].apply(this, [obj]);
};
...
var parse2 = {
  ...
  MultiPoint: function(array) {
    ...
    var points = [];
    for (var i = 0; i < array.length; ++i) {
      points.push(parse2.Point.apply(this$1$1, [array[i]]));
    }
  ...
  MultiLineString: function(array) {
    ...
    var lineStrings = [];
    for (var i = 0; i < array.length; ++i) {
      lineStrings.push(parse2.LineString.apply(this$1$1, [array[i]]));
    }
  ...
  Polygon: function(array) {
    ...
    for (var i = 1; i < array.length; ++i) {
      var hole = array[i];
      var coordinates = parse2.coordinates.apply(this$1$1, [hole]);
  ...
  MultiPolygon: function(array) {
    ...
    for (var i = 0; i < array.length; ++i) {
      ...
      polygons.push(parse2.Polygon.apply(this$1$1, [polygon4]));
    }
...
var extract$1$1 = {
  ...
  Point: function(point4) {
    var array = extract$1$1.coordinate.apply(this, [point4.getCoordinate()]);
    return {
      type: "Point",
      coordinates: array
    };
  ...
  MultiPoint: function(multipoint3) {
    ...
    for (var i = 0; i < multipoint3._geometries.length; ++i) {
      ...
      var geoJson = extract$1$1.Point.apply(this$1$1, [point4]);
  ...
  LineString: function(linestring3) {
    ...
    for (var i = 0; i < coordinates.length; ++i) {
      ...
      array.push(extract$1$1.coordinate.apply(this$1$1, [coordinate2]));
    }
    return {
      type: "LineString",
      coordinates: array
  ...
  MultiLineString: function(multilinestring3) {
    ...
    for (var i = 0; i < multilinestring3._geometries.length; ++i) {
      ...
      var geoJson = extract$1$1.LineString.apply(this$1$1, [linestring3]);
      array.push(geoJson.coordinates);
    }
    return {
      type: "MultiLineString",
  ...
  Polygon: function(polygon4) {
    var this$1$1 = this;
    var array = [];
    var shellGeoJson = extract$1$1.LineString.apply(this, [polygon4._shell]);
  ...
  MultiPolygon: function(multipolygon3) {
    ...
    for (var i = 0; i < multipolygon3._geometries.length; ++i) {
      ...
      var geoJson = extract$1$1.Polygon.apply(this$1$1, [polygon4]);
      array.push(geoJson.coordinates);
    }
    return {
      type: "MultiPolygon",
...
BufferSubgraph.prototype.getEnvelope = function getEnvelope3() {
  if (this._env === null) {
...
TopologyLocation.prototype.get = function get3(posIndex) {
...
var EdgeRing$1 = function EdgeRing3() {
  this._startDe = null;
...
EdgeRing$1.prototype.getCoordinate = function getCoordinate5(i) {
...
EdgeRing$1.prototype.isHole = function isHole2() {
  return this._isHole;
};
...
EdgeRing$1.prototype.toPolygon = function toPolygon2(geometryFactory) {
  var this$1$1 = this;
...
EdgeRing$1.prototype.interfaces_ = function interfaces_53() {
  return [];
};
EdgeRing$1.prototype.getClass = function getClass52() {
  return EdgeRing$1;
};
...
var Node$2 = (function(GraphComponent$$1) {
  ...
  Node2.prototype.add = function add15(e) {
...
PolygonBuilder.prototype.findEdgeRingContaining = function findEdgeRingContaining2(testEr, shellList) {
...
LineSegment.prototype.intersection = function intersection5(line2) {
...
MonotoneChain.prototype.getEnvelope = function getEnvelope4() {
  if (this._env === null) {
...
var MCIndexNoder = (function(SinglePassNoder$$1) {
  ...
  MCIndexNoder2.prototype.add = function add15(segStr) {
    var this$1$1 = this;
...
Triangle$1.interpolateZ = function interpolateZ2(p, v0, v1, v2) {
  var x0 = v0.x;
  var y0 = v0.y;
  var a = v1.x - x0;
  var b = v2.x - x0;
...
Triangle$1.centroid = function centroid3(a, b, c) {
  var x = (a.x + b.x + c.x) / 3;
  var y = (a.y + b.y + c.y) / 3;
  return new Coordinate(x, y);
};
...
EdgeList.prototype.get = function get4(i) {
...
var Edge$1 = (function(GraphComponent$$1) {
  ...
  Edge.prototype.getDepth = function getDepth3() {
    return this._depth;
  };
  ...
  Edge.prototype.isIsolated = function isIsolated2() {
    return this._isIsolated;
  };
  Edge.prototype.getCoordinates = function getCoordinates11() {
    return this.pts;
  };
  Edge.prototype.setIsolated = function setIsolated(isIsolated2) {
    this._isIsolated = isIsolated2;
  };
  Edge.prototype.setName = function setName(name) {
    this._name = name;
  };
  Edge.prototype.equals = function equals10(o) {
    var this$1$1 = this;
  ...
  Edge.prototype.getCoordinate = function getCoordinate18() {
    if (arguments.length === 0) {
  ...
  Edge.prototype.print = function print9(out) {
    var this$1$1 = this;
  ...
  Edge.prototype.isClosed = function isClosed5() {
  ...
  Edge.prototype.getEnvelope = function getEnvelope5() {
    var this$1$1 = this;
    if (this._env === null) {
  ...
  Edge.prototype.addIntersection = function addIntersection3(li, segmentIndex, geomIndex, intIndex) {
    var intPt = new Coordinate(li.getIntersection(intIndex));
    var normalizedSegmentIndex = segmentIndex;
  ...
  Edge.prototype.toString = function toString25() {
    var this$1$1 = this;
    var buf = new StringBuffer();
  ...
  Edge.prototype.setDepthDelta = function setDepthDelta(depthDelta2) {
    this._depthDelta = depthDelta2;
  };
  Edge.prototype.getEdgeIntersectionList = function getEdgeIntersectionList() {
    return this.eiList;
  };
  Edge.prototype.addIntersections = function addIntersections3(li, segmentIndex, geomIndex) {
    var this$1$1 = this;
    for (var i = 0; i < li.getIntersectionNum(); i++) {
      this$1$1.addIntersection(li, segmentIndex, geomIndex, i);
    }
  };
  Edge.prototype.interfaces_ = function interfaces_170() {
    return [];
  };
  Edge.prototype.getClass = function getClass169() {
    return Edge;
  };
  Edge.updateIM = function updateIM2() {
    if (arguments.length === 2) {
      var label = arguments[0];
      var im = arguments[1];
...
BufferDistanceValidator.prototype.isValid = function isValid2() {
...
BufferResultValidator.prototype.isValid = function isValid3() {
  this.checkPolygonal();
  if (!this._isValid) {
    return this._isValid;
  }
...
BufferResultValidator.isValid = function isValid4(g, distance11, result) {
  var validator = new BufferResultValidator(g, distance11, result);
  if (validator.isValid()) {
    return true;
  }
...
FastNodingValidator.prototype.isValid = function isValid5() {
  this.execute();
  return this._isValid;
};
...
GeometryCollectionMapper.map = function map3(gc, op) {
...
GeometryTransformer.prototype.copy = function copy7(seq) {
  return seq.copy();
};
...
IntervalIndexedGeometry.prototype.addLine = function addLine(pts) {
  ...
  for (var i = 1; i < pts.length; i++) {
    ...
    this$1$1._index.insert(min4, max3, seg);
  }
};
...
var GeometryGraph = (function(PlanarGraph$$1) {
  ...
  GeometryGraph2.prototype.addLineString = function addLineString2(line2) {
    var coord = CoordinateArrays.removeRepeatedPoints(line2.getCoordinates());
    if (coord.length < 2) {
  ...
  GeometryGraph2.prototype.add = function add15() {
    if (arguments.length === 1) {
      var g = arguments[0];
      if (g.isEmpty()) {
        return null;
...
GeometryMapper.map = function map4() {
...
OverlayResultValidator.prototype.isValid = function isValid6(overlayOp3) {
  this.addTestPts(this._geom[0]);
  this.addTestPts(this._geom[1]);
  var isValid8 = this.checkValid(overlayOp3);
  return isValid8;
};
...
OverlayResultValidator.prototype.isValidResult = function isValidResult(overlayOp3, location) {
  ...
  var isValid8 = !(expectedInterior ^ resultInInterior);
  if (!isValid8) {
...
OverlayResultValidator.isValid = function isValid7(a, b, overlayOp3, result) {
...
var bisector = function(compare10) {
  ...
  return {
    left: function(a, x, lo, hi) {
      ...
      while (lo < hi) {
        var mid = lo + hi >>> 1;
        if (compare10(a[mid], x) < 0) lo = mid + 1;
        else hi = mid;
      }
      return lo;
    },
...
function intersect$2(poly1, poly2) {
  ...
  var intersection8 = OverlayOp.intersection(a, b);
  if (intersection8.isEmpty()) return null;
  var writer = new GeoJSONWriter();
  var geom = writer.write(intersection8);
  return feature(geom);
}
const triangulate = (polygon4) => {
  const data = earcut$1.flatten(polygon4.geometry.coordinates);
...
const triangulate$1 = (geojson) => {
  const polygons = [];
  flattenEach(geojson, (feature2) => {
    const triangles = triangulate(feature2);
...
const clip = (subject, clipper2) => {
  const triangles = [];
  const intersection8 = clipIntersection(subject, clipper2);
  if (intersection8) {
    Array.prototype.push.apply(triangles, intersection8);
  }
  const difference4 = clipDifference(subject, clipper2);
  if (difference4) {
    Array.prototype.push.apply(triangles, difference4);
  }
...
const clipDifference = (subject, clipper2) => {
  const triangles = [];
  try {
    const difference$1 = difference3(subject, clipper2);
    if (difference$1 === null) {
      return null;
    }
    flattenEach(difference$1, (flattened) => {
      featureEach$1(triangulate$1(flattened), (feature2) => {
...
const clipIntersection = (subject, clipper2) => {
  const triangles = [];
  try {
    const intersection8 = intersect$2(subject, clipper2);
    if (intersection8 === null) {
      return null;
    }
    if (intersection8 && intersection8.geometry.type.includes("Polygon")) {
      flattenEach(intersection8, (flattened) => {
        featureEach$1(triangulate$1(flattened), (feature2) => {
          triangles.push(feature2);
        });
      });
...
const segment = (subject, clippers) => {
  ...
  coordEach$1(intersections, (coordinate2) => {
    vertices.push(coordinate2);
  });
  coordEach$1(clippers, (coordinate2) => {
    if (booleanPointOnLine(coordinate2, subject, {
...
const similar = (array1, array2, start1 = 0, start2 = 0, count2 = -1, reverse4 = false) => {
  ...
  count2 = Math.min(length, Math.max(count2, 0));
  for (let i = 0; i < count2; i = i + 1) {
    const delta = reverse4 ? -i : i;
    const index1 = (start1 + i + length) % length;
    const index2 = (start2 + delta + length) % length;
    const isEqual2 = array1[index1].every((dimension, j) => {
...
function quickselectStep(arr, k, left, right, compare10) {
  while (right > left) {
    ...
    var j = right;
    swap2(arr, left, k);
    if (compare10(arr[right], t) > 0) swap2(arr, left, right);
    while (i < j) {
      swap2(arr, i, j);
      i++;
      j--;
    ...
    if (compare10(arr[left], t) === 0) swap2(arr, left, j);
    else {
      j++;
      swap2(arr, j, right);
    }
...
rbush.prototype = {
  ...
  search: function(bbox2) {
    var node = this.data, result = [], toBBox = this.toBBox;
    if (!intersects6(bbox2, node)) return result;
    var nodesToSearch = [], i, len, child, childBBox;
    while (node) {
      for (i = 0, len = node.children.length; i < len; i++) {
        child = node.children[i];
        childBBox = node.leaf ? toBBox(child) : child;
        if (intersects6(bbox2, childBBox)) {
          if (node.leaf) result.push(child);
  ...
  collides: function(bbox2) {
    var node = this.data, toBBox = this.toBBox;
    if (!intersects6(bbox2, node)) return false;
    var nodesToSearch = [], i, len, child, childBBox;
    while (node) {
      for (i = 0, len = node.children.length; i < len; i++) {
        child = node.children[i];
...
function coordEach(geojson, callback, excludeWrapCoord) {
  if (geojson === null) return;
  var j, k, l, geometry, stopG, coords, geometryMaybeCollection, wrapShrink = 0, coordIndex = 0, isGeometryCollection2, type2 = geojson.type, isFeatureCollection = type2 === "FeatureCollection", isFeature = type2 === "Feature", stop = isFeatureCollection ? geojson.features.length : 1;
  for (var featureIndex = 0; featureIndex < stop; featureIndex++) {
    geometryMaybeCollection = isFeatureCollection ? geojson.features[featureIndex].geometry : isFeature ? geojson.geometry : geojson;
    isGeometryCollection2 = geometryMaybeCollection ? geometryMaybeCollection.type === "GeometryCollection" : false;
    stopG = isGeometryCollection2 ? geometryMaybeCollection.geometries.length : 1;
...
const boundaries = (linestring3, polygons) => {
  let linePartition = linestring3;
...
const isInLine = (geojson, lines, boundary, every, within2) => {
  ...
  return reducerGeo(geojson, (coordinate2) => {
    return reducerLines(lines, (linestring3) => {
      return booleanPointOnLine(coordinate2, linestring3, {
...
const isInPoint = (geojson, points, every, within2) => {
  ...
  return reducerGeo(geojson, (coordinate2) => {
    const point1 = point$2(coordinate2);
    return reducerPts(points, (point22) => {
      return point1.geometry.coordinates.every((dimension, index2) => {
        return dimension === point22.geometry.coordinates[index2];
      }) === within2;
    });
  });
};
const isInPolygon$1 = (geojson, polygons, boundary, every, within2) => {
  ...
  return reducerGeo(geojson, (coordinate2) => {
    return reducerPoly(polygons, (polygon4) => {
      return booleanPointInPolygon(coordinate2, polygon4, {
...
const end = (linestring3) => {
  const length = linestring3.geometry.coordinates.length;
  return point$2(linestring3.geometry.coordinates[length - 1]);
};
...
const isIntersecting = (lines1, lines2, boundary) => {
  ...
  return meta.flattenSome(lines1, (line1) => {
    return meta.flattenSome(lines2, (line2) => {
      const intersects7 = lineIntersect(line1, line2);
      if (boundary === false) {
        const lines = featureCollection([line1, line2]);
        if (isOnBoundary(lines, intersects7)) {
          return false;
        }
      }
      return intersects7.features.length > 0;
    });
  });
};
const isOnBoundary = (lines, points) => {
  return meta.featureEvery(points, (pt) => {
    return meta.flattenSome(lines, (linestring3) => {
      const lineStart = start(linestring3);
      const lineEnd = end(linestring3);
...
const start = (linestring3) => {
  return point$2(linestring3.geometry.coordinates[0]);
};
const line$1 = {
  end,
  isDisjoint,
...
const isBoundaryIntersecting = (polygons1, polygons2) => {
  return meta.flattenSome(polygons1, (polygon1) => {
    ...
    return meta.flattenSome(polygons2, (polygon22) => {
      const boundary2 = polygonToLine(polygon22);
...
const withinPolygon$2 = (lines, polygons) => {
  ...
  return util.meta.flattenEvery(lines, (linestring3) => {
    const linePartition = util.partition.boundaries(linestring3, polygons);
...
const within = (geojson1, geojson2, error = true) => {
  ...
  switch (type3) {
    ...
    case "Point-LineString":
      return point3.withinLine(geojson1, geojson2);
    case "Point-Point":
      return point3.withinPoint(geojson1, geojson2);
    case "Point-Polygon":
      return point3.withinPolygon(geojson1, geojson2);
    case "Polygon-Polygon":
      return polygon3.withinPolygon(geojson1, geojson2);
    default:
      if (error) {
...
function geoWithin(prop, query5) {
  try {
    return index.within(prop, bboxToGeojson(query5), false);
  } catch (e) {
    return false;
  }
}
...
function tlMatches(doc, query5) {
  var key = Object.keys(query5)[0];
  var value = query5[key];
  if (key.charAt(0) == "$") {
...
function matches(doc, query5) {
  return and(doc, toArray(query5));
}
function applyUpdates(updates, doc, setOnInsert) {
  ...
  for (var i = 0; i < keys2.length; i++) {
    ...
    if (key == "$inc") {
      ...
      for (var j = 0; j < fields.length; j++) {
        var field = fields[j];
        var values3 = value[field];
        for (var k = 0; k < values3.length; k++) {
          doc[field].push(values3[k]);
        }
      }
    } else if (key == "$push") {
      var fields = Object.keys(value);
...
function createDocFromUpdate(query5, updates, idGenerator) {
  ...
  if (onlyFields) {
    ...
    var queryKeys = Object.keys(query5);
    for (var i = 0; i < queryKeys.length; i++) {
      newDoc[queryKeys[i]] = query5[queryKeys[i]];
    }
...
class Collection2 {
        ...
        for (const groupKey in groups) {
          ...
          for (const field in stageSpec) {
            ...
            if (accType === "$sum") {
              ...
              result[field] = count2 > 0 ? sum / count2 : 0;
            } else if (accType === "$min") {
              let min4 = void 0;
              for (let k = 0; k < group.docs.length; k++) {
                if (typeof accExpr === "string" && accExpr.charAt(0) === "$") {
                  const val = getProp(group.docs[k], accExpr.substring(1));
                  if (val !== void 0 && (min4 === void 0 || val < min4)) {
                    min4 = val;
                  }
                }
              }
              ...
              for (let k = 0; k < group.docs.length; k++) {
                if (typeof accExpr === "string" && accExpr.charAt(0) === "$") {
                  const val = getProp(group.docs[k], accExpr.substring(1));
                  if (val !== void 0 && (max3 === void 0 || val > max3)) {
                    max3 = val;
                  }
                }
              }
  ...
  find(query5, projection) {
    return new Cursor(
      this,
      query5 == void 0 ? {} : query5,
      projection,
      matches,
      this.storage,
      this.indexes,
  ...
  findOneAndUpdate(filter17, update, options) {
    ...
    if (!c.hasNext()) return null;
    const doc = c.next();
    const clone5 = Object.assign({}, doc);
    applyUpdates(update, clone5);
    this.storage.set(doc._id, clone5);
    if (options && options.returnNewDocument) {
      if (options && options.projection) return applyProjection(options.projection, clone5);
      else return clone5;
    } else {
      if (options && options.projection) return applyProjection(options.projection, doc);
      else return doc;
  ...
  update(query5, updates, options) {
    ...
    if (c.hasNext()) {
      ...
      if (options && options.upsert) {
        const newDoc = createDocFromUpdate(query5, updates, this.idGenerator);
        this.storage.set(newDoc._id, newDoc);
...
class DB {
  constructor(options) {
    ...
    if (typeof localStorage !== "undefined") {
      this.localStorage = new Collection2(
        this,
